"""
--- SCRIPT: search_index_master.py ---
PURPOSE: Indexes 3D asset files and builds a searchable metadata parquet and CSV for Everything Search and CLIP-powered scripts.

USAGE:
    1. Place your asset files in the appropriate directories.
    2. Run this script to generate or update G:/_index.parquet and G:/everything_metadata.csv.
    3. Other search scripts will use these files for fast lookups.

DEPENDENCIES: pandas, torch, open_clip, PIL, tqdm
"""
...
def update_everything_display(df):
    ...
    # Everything's star property uses 19/39/59/79/99 and blank for unrated.
    export_df['Rating'] = export_df['Rating'].apply(to_everything_rating)
    
    # Save the CSV that Everything 1.5 is "watching"
    # (Image files only - zip linking is handled in search_tag_assets.py metadata.efu export)
