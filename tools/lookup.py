...
def load_crm_data():
    ...
    if not os.path.exists(CRM_PATH): return None
...
def get_crm_tag(name, email, crm_data):
    ...
    if not n and not e: return ""
    if e and e not in crm_data['emails']: return " \033[1;41m[! EMAIL NOT IN CRM]\033[0m"
    if n and n not in crm_data['names'] and n not in crm_data['organizations']: return " \033[1;41m[! NAME NOT IN CRM]\033[0m"
    return " \033[92m[✓ CRM]\033[0m"

def print_row(label, value, width=15):
    if not value or value == "-": disp = "\033[90m-\033[0m"
    else: disp = str(value)
...
def show_contacts(data, crm):
    if "contacts" not in data: return
...
def show_docs(data):
    if "design_documents" not in data: return
...
def show_deliverables(data, key, title):
    ...
    for k, v in sorted(items.items()):
        if isinstance(v, dict):
            ...
            # Parse legacy "Status (Description)" format
            val_str = str(v).strip()
        ...
        s_color = "\033[92m" if s_lower == 'final' else "\033[32m" if s_lower == 'active' else "\033[33m" if s_lower == 'draft' else "\033[90m"
        
        # ID (Cyan) | Description (White) | Status (Color) | Comment (Gray)
        print(f"  \033[36m{k:<5}\033[0m \033[37m{desc:<30}\033[0m {s_color}{status:<10}\033[0m \033[90m{comment}\033[0m")

def main():
    if len(sys.argv) < 2:
        print("\033[91mUsage:\033[0m rr p [CODE] [docs|contacts|links|full]")
        sys.exit(1)
    ...
    if view == "summary":
        # Show a tiny status recap for summary view
        ...
        # Show rendering summary
        rends = data.get('renderings', {})
        anis = data.get('animations', {})
        if rends or anis:
            all_vals = list(rends.values()) + list(anis.values())
            active = sum(1 for v in all_vals if v.lower() in ['active', 'final', 'confirmed'])
            print_row("Deliverables", f"{active}/{len(all_vals)} Active/Final")

        print(f"\033[90m\nRun 'rr p {code} [docs|contacts|links|full]' for more.\033[0m")
    
    elif view == "contacts": show_contacts(data, crm)
    elif view == "docs": show_docs(data)
    elif view == "links": show_links(data)
    elif view in ["renderings", "rend"]: show_deliverables(data, 'renderings', 'RENDERINGS')
    elif view in ["animations", "ani"]: show_deliverables(data, 'animations', 'ANIMATIONS')
    elif view == "full":
